...
{
    u16 lo = ReadBus(address); //reads low byte of 16bit
    u16 hi = ReadBus(address + 1); //reads the high bit of 16bit

    return lo | (hi << 8); //returns the low byte OR the high byte shifted 8 to the left
}
